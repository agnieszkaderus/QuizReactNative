...
import {
  ...
  Animated,
  Modal,
  SafeAreaView, ActivityIndicator, TextInput, TouchableHighlight, FlatList, RefreshControl, Button
} from 'react-native'
import {useEffect, useState} from "react";
import MaterialCommunityIcons from "react-native-vector-icons/MaterialCommunityIcons";
import Header from "../components/headers/header";
import ThirdQuizScreen from "./ThirdQuizScreen";
...
const TestsList = ({navigation, onPress}) => {
  ...
  const ItemRender = ({ id, name, level, numberOfTasks}) => {
    return (
        <TouchableHighlight onPress={()=>{
          setQuizID(id);
          navigation.navigate("ThirdQuizScreen");
          cons(id);
        }}>
          <View style={styleSheet.listItem}>
            <Text style={styleSheet.itemTextBold}> {name} </Text>
            <Text style={styleSheet.itemText}> level: {level}</Text>
            <Text style={styleSheet.itemText}> questions: {numberOfTasks}</Text>
    ...
    return (
        <View>
          <View style={styles.questionPlace}>
            <Text style={styles.questionText}>  {indexOf(allQuestions)}. {allQuestions[currentQuestionIndex]?.question} </Text>
          </View>
  ...
  return (
      ...
      <ActivityIndicator
        ...
        <FlatList
            ...
            ListHeaderComponent={ <Header text={"TESTS"} />}
            refreshControl={
              <RefreshControl
                  refreshing={onRefresh}
                  onRefresh={()=> {
                    Call_RefreshControl
                    cons("Refresh control w quizach")}}
                  tintColor="#C51162"
                  title='Loading...'
                  titleColor='blue'
              />
